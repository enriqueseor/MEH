...
public class Map implements ISaveable {
	...
	public SpriteHeader.SpritesSignManager mapSignManager;
	public SpriteExitManager mapExitManager;
	...
	public Map(GBARom rom, int bank, int map) {
		this(rom, BitConverter.shortenPointer(BankLoader.maps[bank].get(map)));
	}
	...
	public Map(GBARom rom, int dataOffset) {
		...
		mapSprites = new SpriteHeader(rom, (int) mapHeader.pSprites);
		mapNPCManager=new SpriteNPCManager(rom, this, (int) mapSprites.pNPC, mapSprites.bNumNPC);
		mapSignManager = new SpriteHeader.SpritesSignManager(rom, this, (int)mapSprites.pSigns, mapSprites.bNumSigns);
		mapTriggerManager = new TriggerManager(rom, this, (int) mapSprites.pTraps, mapSprites.bNumTraps);
		mapExitManager = new SpriteExitManager(rom, this, (int) mapSprites.pExits, mapSprites.bNumExits);
	...
	public static Image renderMap(Map map, boolean full) {
		...
		try {
			imgBuffer = new BufferedImage((int) map.getMapData().mapWidth * 16, (int) map.getMapData().mapHeight * 16, BufferedImage.TYPE_INT_ARGB);
